...
**Understand how a coding agent works: read the architecture, run the Python labs, and follow the loop from a prompt to a tool result.**
...
[简体中文 README](./README.md) · [Experiments](./experiments/) · [English docs overview](./docs/en/00-overview.md)

</div>

---

**17 bilingual chapters · 15 Python labs · 5 small examples · Start with no API key**

How does an agent decide to call a tool, ask for permission, or compress a long conversation? This independent learning lab connects each question to an architecture chapter and a runnable experiment.

[Run your first lab](#getting-started) · [Choose a learning track](#learning-tracks) · [中文文档](./docs/zh/00-overview.md)
...
├── README_ZH.md                 # Chinese README (compatibility entry)
...
### 1. Run the core loop without an API key

Requires **Python 3.11+**. This Mock experiment uses only the standard library.
...
The first query calls a calculator, receives `14`, and completes on turn 2. Mock responses are scripted: use this mode to observe control flow, not model capability.

Read [the core loop chapter](./docs/en/03-core-loop.md) and [its lab guide](./docs/en/experiments/03-core-agent-loop-lab.md) alongside the output.

### 2. Install dependencies for more experiments or real APIs
...
Set `ANTHROPIC_API_KEY` before using Anthropic, or use `--provider openai` with `OPENAI_API_KEY` and optionally `OPENAI_BASE_URL`.

On macOS/Linux, you can also run these commands from the repository root:
...
**[简体中文 README](./README.md)** · Happy learning
